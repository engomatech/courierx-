...
// ── GET /api/v1/admin/customers/pending-verification ─────────────────────────
// Lists every portal registrant who has never completed OTP verification.
// These are real DB rows (registration succeeded) but email_verified = 0.
router.get('/customers/pending-verification', (req, res) => {
  try {
    const rows = db.prepare(`
...
// ── POST /api/v1/admin/customers/nudge-pending ────────────────────────────────
// Issues a fresh 24-hour OTP to every unverified portal registrant and emails
// them. Safe to call multiple times — existing verified customers are skipped.
router.post('/customers/nudge-pending', async (req, res) => {
  try {
